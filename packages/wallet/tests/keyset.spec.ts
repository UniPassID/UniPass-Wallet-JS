...
describe("Test Keyset", () => {
  it("Create Keyset Should Success", async () => {
    ...
    const keyset = Keyset.create(registerEmail, registerEmailPepper, masterKey, [guardian1, guardian2]);
    ...
    expect(keyset.keys[1]).toEqual(
      new KeyEmailDkim("Raw", registerEmail, hexlify(registerEmailPepper), new RoleWeight(60, 0, 60)),
    );
